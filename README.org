...
+ *At-a-glance overview of your appointments and free timeslots when used with* [[https://github.com/ichernyshovvv/org-timeblock][org-timeblock]]
...
** Dependencies
 ...
 - [[https://github.com/eschulte/emacs-web-server][emacs-web-server]] (if you experience a conflict, see: [[#known-issues][Known issues]])
...
=org-outlook-tenant-id= to restrict the scope of your app to your Azure tenant).
...
Under "Authentication" click =Add a platform= and add =http://localhost:9004= as
...
*** Fixed Issues
+ *Issue [[https://github.com/ifitzpat/org-outlook/issues/1][#1]] - DST Duplicates*: Fixed duplicate events during daylight saving time transitions
...
+ *Issue [[https://github.com/ifitzpat/org-outlook/issues/2][#2]] - Refresh Token*: Fixed reversed logic that prevented automatic OAuth flow when refresh token expired
+ *Issue [[https://github.com/ifitzpat/org-outlook/issues/3][#3]] - Auth Code Race Conditions*: Removed blocking sleeps and race conditions from OAuth flow
  - Non-blocking authentication with 5-minute timeout (up from 90 seconds)
  - Proper async handling with state tracking
  - Better user feedback and error messages
+ *Issue [[https://github.com/ifitzpat/org-outlook/issues/4][#4]] - Deleted Meetings*: Implemented intelligent hybrid sync
...
** Known issues
 ...
 - Accepting an event doesn't change the event status from =REQUEST= to =MEETING= until the
   next sync.
 - This package depends on [[https://github.com/eschulte/emacs-web-server][emacs-web-server]] which conflicts with
   [[https://github.com/skeeto/emacs-web-server][skeeto/emacs-web-server]]. A workaround is mentioned in [[https://github.com/ifitzpat/org-outlook/issues/10][#10]].
