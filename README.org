...
+ *Intelligent Hybrid Sync*
  - Fast delta sync for daily use (only syncs changes)
  - Automatic full sync for validation (weekly by default)
  - Handles new, updated, and deleted events automatically
...
+ *Robust OAuth flow* with non-blocking authentication and proper error handling
...
** Fixed in 0.0.6-alpha

 - ✅ DST duplicate events (fixed in 0.0.6)
 - ✅ Refresh token timeout handling (fixed in 0.0.6)
 - ✅ OAuth authentication race conditions (fixed in 0.0.6)
 - ✅ Deleted meetings not removed from org mode (fixed in 0.0.6)
